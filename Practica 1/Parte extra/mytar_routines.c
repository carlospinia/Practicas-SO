...
int extractTarDirectory (char* tarName, char* directory){
	FILE *tarFile = fopen(tarName, "r");
		...
		int i;
		for (i = 0; i < nFiles; i++) {
			char *nombreFichero = (char *) malloc(sizeof(directory)*sizeof(p[i].name));
			char *nombre = p[i].name;
			sprintf(nombreFichero, "%s/%s", directory, nombre);
			//Creamos nuevo fichero con mismo nombre en modo escritura, leemos el contenido y lo copiamos en el nuevo fichero
			FILE *nuevo = fopen(nombreFichero, "w");
			if (nuevo != NULL) {
				c = (unsigned char*) malloc(p[i].size);
				fread(c, sizeof(unsigned char) * p[i].size, 1, tarFile); //leemos de tarFile
				fwrite(c, sizeof(unsigned char) * p[i].size, 1, nuevo);	//escribimos en el nuevo archivo
...
int listTar(char* tarName){
	...
	for (i = 0; i < nFiles; i++){
		printf("[%i] File name: %s , size: %i bytes\n", i, p[i].name, p[i].size);
		//printf("%i\n", p[i].size);
	}
	return 0;
}
