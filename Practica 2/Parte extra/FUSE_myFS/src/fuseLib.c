...
static int my_getattr(const char *path, struct stat *stbuf)
{
	...
	if (node->fileType == 1)
        ...
        stbuf->st_nlink = node->numEnlaces;
        stbuf->st_uid = getuid();
        stbuf->st_gid = getgid();
...
static int my_mknod(const char *path, mode_t mode, dev_t device)
{
	...
	myFileSystem.nodes[idxNodoI]->numEnlaces = 1;
	myFyleSystem.nodes[idxNodoI]->fileType = 0;
...
static int my_unlink(const char *path) {
	 ...
	 */
	int result = 0;
		int dirPosition = findFileByName(&myFileSystem, (char*)path+1);
		if(dirPosition != -1){ //existe el directorio
			int idxNode = myFileSystem.directory.files[dirPosition].nodeIdx; //1
			resizeNode(idxNode, 0); //2
			myFileSystem.nodes[idxNode]->freeNode = true; //3
			myFileSystem.directory.files[dirPosition].freeFile = true; //4
			myFileSystem.numFreeNodes++;
			myFileSystem.directory.numFiles--; //5

			int i;
			int numBlocks = myFileSystem.nodes[idxNode]->numBlocks;
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *f) {
	...
	//Comprobar offset < fihero
	//bytes2Read = num bytes a leer
	...
	while (totalRead < bytes2Read) {
		...
		currentBlock = node->blocks[offset/BLOCK_SIZE_BYTES];
		offBlock = offset % BLOCK_SIZE_BYTES;
		//Fallo en la lectura del bloque al hace lseek o read
		...
		if ((lseek(myFileSystem.fdVirtualDisk, currentBlock * BLOCK_SIZE_BYTES, SEEK_SET) == (off_t) - 1) ||
			...
			perror("Block could not be read.");
			return -EIO;
		}

		int i = offBlock;
		for(; (i < BLOCK_SIZE_BYTES) && (totalRead < bytes2Read); i++)
			buf[totalRead++] = buffer[i];

		//Fallo en la escritura
		if((lseek(myFileSystem.fdVirtualDisk, currentBlock * BLOCK_SIZE_BYTES, SEEK_SET) == (off_t) - 1) ||
			(write(myFileSystem.fdVirtualDisk, &buffer, BLOCK_SIZE_BYTES) == -1)){
			perror("Buffer could not be written");
			return -EIO;
		}

	    offset = offset + i;
		bytes2Read = bytes2Read - (i-offBlock);
	}
...
int my_rename(const char *oldfile, const char *newfile){
	int idFilePosition = 0;
	if ( (idFilePosition = findFileByName(&myFileSystem, (char*)oldfile+1)) == -1){
		return -ENOENT;
	}
	if (findFileByName(&myFileSystem, (char*)newfile+1) != -1){
		my_unlink(newfile);
	}
	FileStruct* file = &myFileSystem.directory.files[idFilePosition];
	strcpy(file->fileName, (char*)newfile+1);
	updateDirectory(&myFileSystem);
	sync();
...
int my_readlink(const char *path, char *buf, size_t bufsize)
{
	...
	NodeStruct* inode=NULL;
	char* name=(char*)path+1;
	int idxInode;

	/* Search directory entry for the file */
	ent_idx=findFileByName(&myFileSystem,name);

	if (ent_idx==-1)
		return -ENOENT;

	directoryEntry=&myFileSystem.directory.files[ent_idx];
	idxInode=directoryEntry->nodeIdx;
	inode=myFileSystem.nodes[idxInode];

	if (inode->fileType!=1)
		return -EINVAL;

	if (bufsize<strlen(inode->slinkDestination))
		strcpy(buf,inode->slinkDestination);
	else {
		strncpy(buf,inode->slinkDestination,bufsize-1);
		buf[bufsize-1]='\0';
	}
...
static int my_symlink (const char *path){
